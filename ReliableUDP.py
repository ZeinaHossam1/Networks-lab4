...
class ReliableUDP:
    ...
    def build_packet(self, seq_num, ack_num, syn_flag, ack_flag, fin_flag, data=b""):
        ...
        dummy_header = struct.pack(
            header_format, seq_num, ack_num, syn_flag, ack_flag, fin_flag, 0
        )
        dummy_packet = dummy_header + data  # To calculate checksum for everything

        value = self.calculate_checksum(dummy_packet)
        header = struct.pack(
            header_format, seq_num, ack_num, syn_flag, ack_flag, fin_flag, value
        )
        packet = header + data
    ...
    def connect(self, server_address):
        ...
        while retries < max_retries:
            try:
                ...
                print(
                    f"Sent SYN packet. Waiting for SYN-ACK... (Attempt {retries + 1})"
                )
                # (2) waiting for SYN-ACK packet
                r_data, addr, r_seq, r_ack, r_syn, r_ack_flag, r_fin = (
                    self.receive()
                )  # If an ACK wasn't received in 2 secs a timeout occurs and it returns an error
    ...
    def receive(self, buffer_size=1024):
        while True:
            ...
            try:
                raw_packet, sender_address = self.socket.recvfrom(
                    buffer_size
                )  # Catches the entire combined packet
            ...
            data = raw_packet[15:]
            seq, ack, syn, ack_flag, fin, received_checksum = struct.unpack(
                "!IIBBBI", header
            )
